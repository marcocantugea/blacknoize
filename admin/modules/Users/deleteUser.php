...
<?php
    ...
    if(isset($_POST)){
        if(isset($_POST['iduser']) && isset($_POST['token'])){
            $token_qs= $_POST['token'];
            $idser_qs=$_POST['iduser'];
            if($token_qs==$token){
                $deluser = new UserObj();
                $deluser->iduser=$idser_qs;
                $_ADOUser = new ADOUsers();
                $_ADOUser->DeleteUser($deluser);
...
<p>&nbsp;</p>
<p>&nbsp;</p>
<form id="deleteUser" method="post" action="deleteUser.php">
